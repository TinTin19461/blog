 ...
 * @since 1.0.0
 */
class Blogistic_WP_Custom_Repeater extends \WP_Customize_Control {
    /**
     ...
     * Main function
     * 
     * Blogistic_WP_Custom_Repeater class name 
     */
    ...
    function enqueue() {
        ...
        wp_enqueue_style( 'blogistic-wp-repeater', get_template_directory_uri() . '/inc/customizer/custom-controls/repeater/repeater.css', array(), BLOGISTIC_VERSION, 'all' );
        wp_enqueue_script( 'blogistic-wp-repeater', get_template_directory_uri() . '/inc/customizer/custom-controls/repeater/repeater.js', array( 'jquery' ), BLOGISTIC_VERSION, true );
    }
    ...
    public function render_content() {
        ...
        $item_count = 1;
        ?>
        <div class="blogistic-repeater-control">
            <label class="customize-control-title"><?php echo esc_html( $this->label ); ?></label>
            <?php if( $this->description ) { ?>
                <span class="description customize-control-description"><?php echo esc_html( $this->description ); ?></span>
            <?php } ?>
            <div class="blogistic-repeater-control-inner">
                <?php 
                $row_label = $this->row_label;
                 foreach( $control_values as $control_value_key => $control_value ) :
                  ...
                  ?>
                    <div class="blogistic-repeater-item <?php if( $control_value->item_option === 'show' ) : echo 'visible'; else : echo 'not-visible'; endif; ?>">
    ...
    function render_control( $field_key, $field, $current_item_value ) {
        $type = $field->type;
        $label = isset( $field->label ) ? $field->label : esc_html__( 'Item Label', 'blogistic' );
        ...
        switch( $type ) {
            ...
            case 'image' : ?>
                <div class="single-control image-field">
                    ...
                    <div class="image-holder">
                        ...
                        <div class="add-image-trigger <?php if( $current_item_value->$field_key ) echo 'no-trigger'; ?>"><?php echo esc_html__( 'Upload Image', 'blogistic'); ?></div>
            ...
            <?php
                break;
            case 'fontawesome-icon-picker' : $icons_list = blogistic_get_all_fontawesome_icons( $families ); ?>
                <div class="single-control fontawesome-icon-picker">
                    ...
                    <div class="icon-holder">
                        ...
                        <div class="icons-list" style="display:none;">
                            <input class="icon-search-input" type="text" placeholder="<?php echo esc_attr__( "Type to search", "blogistic" ); ?>"/>
                            <?php
            ...
            case 'alignment': ?>
                        ...
                        <?php
                            if( ! empty( $options ) ) :
                                foreach( $options as $option_key => $option_value ) :
                                    ...
                                    ?>
                                        <span class="alignment-item<?php echo $is_active_class .' '. $option_key; ?>"><?php echo esc_html( $option_value ); ?></span>
                                    <?php
                                endforeach;
                            endif;
                        ?>
                ...
                break;
            case 'social-share': 
                $icons_list = blogistic_get_all_social_share();
                ?>
                    <div class="single-control fontawesome-icon-picker">
                        <h2 class="control--item-label"><?php echo esc_html( $label ); ?></h2>
                        <?php if( $description ) { ?>
